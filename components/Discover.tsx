...
const Discover: React.FC<DiscoverProps> = ({ vendors, onSelectVendor, favorites, onToggleFavorite }) => {
  ...
  const handleSwipe = (id: string) => {
    // Delay slightly to allow the exit animation to start smoothly
    setTimeout(() => {
        ...
        <div className="flex gap-3">
          {categories.map(cat => (
            <button 
              ...
              className={`px-4 py-2 rounded-full border-2 border-neo-black font-bold text-sm whitespace-nowrap transition-all shadow-hard-sm ${activeCategory === cat ? 'bg-neo-black text-white' : 'bg-white text-neo-black hover:bg-gray-100'}`}
            >
              {cat}
            </button>
          ))}
      ...
      <div className="flex-1 relative w-full max-w-sm mx-auto flex items-center justify-center p-4 flex-shrink-0">
        {filteredVendors.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center z-0 w-full py-20">
            ...
            <button onClick={resetDeck} className="bg-neo-orange px-6 py-3 border-2 border-neo-black shadow-hard font-bold rounded-xl active:translate-y-[2px] active:translate-x-[2px] active:shadow-none transition-all">
              Reset Deck
            </button>
          </div>
        ) : (
...
const Card: React.FC<CardProps> = ({ vendor, isTop, onSwipe, onSelect, isFavorite, onToggleFavorite }) => {
  const x = useMotionValue(0);
  const rotate = useTransform(x, [-200, 200], [-15, 15]);
  const opacity = useTransform(x, [-200, -150, 0, 150, 200], [0, 1, 1, 1, 0]);
  
  // Custom drag constraints and end-drag logic for better "flick" feel
  const handleDragEnd = (event: any, info: any) => {
    const swipeThreshold = 100;
    const velocityThreshold = 800;
    
    // Check if the card was moved far enough OR swiped fast enough
  ...
  return (
    <motion.div
      ...
      // Snap-back transition if released too early
