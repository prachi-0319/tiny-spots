...
const LandingPage: React.FC<LandingPageProps> = ({ onAuth }) => {
  ...
  return (
    <div className="min-h-screen bg-neo-yellow flex flex-col items-center justify-center p-6 font-sans">
      <div className="w-full max-w-sm bg-white border-2 border-neo-black rounded-3xl p-8 shadow-hard-lg">
        ...
        <div className="flex gap-2 mb-6 p-1 bg-gray-100 rounded-xl border-2 border-neo-black">
            <button 
                onClick={() => { setIsLogin(true); setError(null); }}
                className={`flex-1 py-2 rounded-lg font-bold text-sm transition-all ${isLogin ? 'bg-white border-2 border-neo-black shadow-hard-sm' : 'text-gray-500 hover:text-black'}`}
            >
                Login
            </button>
            <button 
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button 
                type="submit" 
                disabled={isLoading}
                className="w-full bg-neo-teal text-white font-bold py-4 rounded-xl border-2 border-neo-black shadow-hard active:translate-y-[2px] active:translate-x-[2px] active:shadow-none transition-all mt-6 disabled:opacity-50"
            >
                {isLoading ? "Checking..." : (isLogin ? "Let's Go!" : "Join the Club")}
            </button>
        </form>
      </div>
    </div>
